...
loop_1: # finding the right place to insert
    ...
    ld s3, 0(s0) # s3=curr->value

    blt s1, s3 , left # val < curr->val move left
    bgt s1, s3 , right # val > curr->right move right
...
loop_2:
    ...
    beq a1, s2, equal # val=curr->val
    blt a1, s2, move_left # val < curr->val , curr=curr->left 
    bgt a1, s2, move_right # val > curr->val, curr=curr->right 
